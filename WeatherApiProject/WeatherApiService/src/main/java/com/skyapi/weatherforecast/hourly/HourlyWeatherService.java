...
public class HourlyWeatherService {
	...
	public List<HourlyWeather> updateByLocationCode(String locationCode, List<HourlyWeather> hourlyForecastInRequest)
		...
		for (HourlyWeather hourlyWeather : hourlyWeatherInDB) {
			if(!hourlyForecastInRequest.contains(hourlyWeather)) {
				hourlyWeatherToBeRemoved.add(hourlyWeather.getShallowCopy());
			}
		}
		for (HourlyWeather hourlyWeather : hourlyWeatherToBeRemoved) {
			hourlyWeatherInDB.remove(hourlyWeather);
		}
